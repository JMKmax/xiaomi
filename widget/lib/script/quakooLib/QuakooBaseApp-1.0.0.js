...
var QuakooDb = (function () {
    ...
    _proto._setItem = function (key, value) {
        ...
        var sql = 'INSERT OR REPLACE INTO '+this.dbName+' (key,value) values (\'' + key + '\',\'' + value + '\')';
        var ret = this.db.executeSqlSync({
            name: this.dbName,
            sql: sql
        });
    ...
    _proto._getItem = function (key) {
        ...
        var sql = 'select value from '+this.dbName+' where key=\'' + key + '\'';
    ...
    _proto._removeItem = function (key) {
        ...
        var sql = 'delete from '+this.dbName+' where key="' + key + '"';
        var ret = this.db.executeSqlSync({
            name: this.dbName,
            sql: sql
        });
...
var QuakooPage = (function () {
    ...
    _proto._getPagerDataFormServer = function (getData, saveToDb, alertServerFail, callback) {
        ...
        api.ajax({
            ...
            if (ret && (ret.code == 400||ret.code == 401 )) {
                quakooUser.removeUserInfo();
                quakooApp.bindPush();
                if(!context.isReLogin){
                    var hadDialog = quakooDb.getItem('hadDialog');
                    if(!hadDialog){
                        context.isReLogin=true;
                        quakooDb.setItem('hadDialog',true);
