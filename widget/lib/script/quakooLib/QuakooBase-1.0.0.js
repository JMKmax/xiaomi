...
var QuakooUtils = (function(){
    ...
    _proto.formatTimeToHoursMinutes = function(time){
        return new Date(time).format("hh:mm");
    };
    _proto.formatTimeToMonthSeconds = function(time){
...
//浏览器视口的高度
    ...
    // 表情
    _proto.emotionImg  = function (newContent){
        var _this = this;
        return formatContent = newContent.replace(/\[(.+?)\]/g,
            function (word) {
                var path = _this.getImagePathFormString(word);
                if (path) {
    ...
    _proto.getImagePathFormString = function (string){
