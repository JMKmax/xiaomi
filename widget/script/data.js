...
datalistModel.prototype = {
    init: function () {
        api.setCustomRefreshHeaderInfo({
            bgColor: (getUserInfo().item == 2)? "#232226":"#fff",
//  		isScale: true,
    		image: {
...
//          textUp: '松开刷新...'
    ...
    getDataByDown: function (callBackOnGetDatas, callBackOnGetCacheDatas) {
        ...
        getData.token = getUserInfo().token;
        getData.cursor = 0;
...
//              },200);
    ...
    getPagerDataFormServer: function (getData, saveToDb, alertServerFail, callback) {
        ...
        api.ajax({
            ...
            if (ret&&ret.code == 400&&isBlack(isLoginRepeat)) {
            	var this_time = new Date().getTime();
            	if (this_time - isLoginRepeat < 1500) {
            		return;
            	}
            	// try{
	            // 	bindPush();
	            ...
	            // }
            	$api.setStorage("isLoginRepeat",new Date().getTime());
	    		showOneDialog("温馨提示","您的账号在其他手机上登录，您被迫下线！","确定",function(){
	    			...
	    			ajaxGetUser(visitorUrl,obj,function(ret,err){
		    			...
		    			$api.setStorage("videoing","");//清空是否正在播放的标记
		               	$api.setStorage("photoBrowser","");//这俩个标示  是用于首页点击安卓返回键
		    			$api.setStorage("activeFrameName","question_0");
		    			$api.setStorage("list_flag","2");
		        		if(ret&&ret.success){
		        			setUserInfo(ret.data);
		        			// bindPush();
		        			closeChatConnect();
		        			openNewWindow("registerLogin","../login/registerLogin.html",{type:"logOut"});
		        			api.execScript({//收起侧滑抽屉
								name:"main",
					            script: "openTab('home')"
					        })
			                api.execScript({
		                    ...
		                    api.execScript({
								name:'main',
								frameName :  $api.getStorage("activeFrameName"),
					            script: "execSetTheme_arc()"
					        })
		                    api.execScript({
								name:"main",
					            script: "loginThenRender()"
					        })
		                    api.execScript({
								name:"main",
								frameName:'arc',
            ...
            if (ret&&ret.code == 401&&isBlack(isLoginRepeat)) {
            	var this_time = new Date().getTime();
            	if (this_time - isLoginRepeat < 1500) {
            		return;
            	}
	            ...
	            // }
	        	$api.setStorage("isLoginRepeat",new Date().getTime());
	    		showOneDialog("温馨提示","您的账号被冻结，您被迫下线！","确定",function(){
...
function ajaxGet(url, getData, callBack,origin) {
    ...
    api.ajax({
        ...
        if (ret&&ret.code == 400&&isBlack(isLoginRepeat)) {
        	...
        	var this_time = new Date().getTime().toString().slice(7,12);
        	if (this_time - isLoginRepeat < 1500) {
        		return;
        	}
        	// try{
            ...
            // 	//TODO handle the exception
            // }
        	$api.setStorage("isLoginRepeat",new Date().getTime().toString().slice(7,12));
    		showOneDialog("温馨提示","您的账号在其他手机上登录，您被迫下线！","确定",function(){
    			var obj = {
					imei: api.deviceId,
					plat: api.systemType,
    			...
    			ajaxGetUser(visitorUrl,obj,function(ret,err){
	        		...
	        		if(ret&&ret.success){
	        			...
	        			if (origin == "main") {
	        				...
	        				openNewWindow("registerLogin","html/login/registerLogin.html",{type:"logOut"});
	        			}else{
	        				openNewWindow("registerLogin","../login/registerLogin.html",{type:"logOut"});
	        			}
	        			api.execScript({//收起侧滑抽屉
						...
						api.execScript({ //登陆后重新刷新激活的卡片页(但没有刷新执行异步刷新)
							name: "main",
							script: "relaodHtml()"
						});
						api.execScript({
							name: 'main',
							script: 'deleteFrameName()'
						})
						api.execScript({//mian的主题
							name: 'main',
							script: 'execSetTheme()'
						});
	                    api.execScript({//首页半弧
						...
						api.execScript({ //卡片的主题
							name: 'main',
							frameName: $api.getStorage("activeFrameName_discover"),
							script: 'execSetTheme_arc()'
						});
	        		}
	        	})
	    	})
    	}else if (ret&&ret.code == 401&&isBlack(isLoginRepeat)) {
    		if (origin == "more") {
        		return;
        	}
    		var this_time = new Date().getTime();
    		...
    		showOneDialog("温馨提示","您的账号被冻结，您被迫下线！","确定",function(){
    			...
    			ajaxGetUser(visitorUrl,obj,function(ret,err){
    				closeLoadingGif();
    				$api.setStorage("isLoginRepeat","");//防止重复弹出这个弹窗
	    			$api.setStorage("videoing","");//清空是否正在播放的标记
		            $api.setStorage("photoBrowser","");//这俩个标示  是用于首页点击安卓返回键
	    			$api.setStorage("activeFrameName","question_0");
	    			$api.setStorage("list_flag","2");
