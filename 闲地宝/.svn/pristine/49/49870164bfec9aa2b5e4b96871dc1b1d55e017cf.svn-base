...
function getContent(authorId,msgContent,encode,isNew){
    ...
    if(isNotBlack(msgContent.messageChat.voice)){
    	...
    	msgContent.messageChat = tryJsonParse(msgContent.messageChat);
    	msgContent.messageChat.ext = tryJsonParse(msgContent.messageChat.ext);
    	if(msgContent.messageChat.ext.type == 1){
            msgContent.messageChat.ext.content = tryJsonParse(msgContent.messageChat.ext.content);
            msgContent.messageChat.ext.content.content = tryJsonParse(msgContent.messageChat.ext.content.content);
            if(msgContent.messageChat.ext.content.content.length>0){
            	for(var i = 0;i < msgContent.messageChat.ext.content.content.length; i++){
            		msgContent.messageChat.ext.content.content[i].url = changeImgBySize(msgContent.messageChat.ext.content.content[i].imgUrl,pwinw*2.5,pwinw*2.5);
            		msgContent.messageChat.ext.content.content[i].width = pwinw*2.5+"px";
            		
                }
            	var data_banner = {
            		banner:msgContent.messageChat.ext.content
            	}
                formatContent = template('scriptDiv_banner', data_banner);
            }else{
            	formatContent='<div class="title">'+msgContent.messageChat.ext.content.title+'</div>'; 
            }
        }else if(msgContent.messageChat.ext.type == 2){
    	    var time = msgContent.time;
            ...
            formatContent="";
        }else if(msgContent.messageChat.ext.type == 3){
	     	if(isNew != "isNew"){
	     		try{
	                overSessionByAnswer();
	            }catch (err){
	     	...
	     	chatBox.hide();
            formatContent="";
       }else if(msgContent.messageChat.ext.type == 4){	     	
	     	chatBox.hide();
            formatContent="";
        }else {
            formatContent="";
