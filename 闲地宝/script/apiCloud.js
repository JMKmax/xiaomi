...
function initFs(callBack){
    ...
    fs.exist({
        path: 'fs://douwentwotwo'
    },function(ret,err){

        if(!ret.exist ){
            fs.createDir({ path:'fs://douwentwotwo'},function(ret,err){
...
//获取图片真实尺寸
function getImageRealSize(url,showWidth,showHieght){
...
function setImgByThisSize(url,showWidth,showHieght){
	if(url.indexOf("http") == 0){
		if(url.indexOf("/guoguo/") != (-1)){
	        var s1=url.split("/guoguo/");
	    }else if(url.indexOf("/storage/") != (-1)){
...
//
//	    	return changeImgBySize(url,showWidth);
//	    }

	    var width = Math.round(showWidth*2);

    	var imgTypes = url.substring(url.lastIndexOf("."));
    	if (showWidth > orgWidth) {
    		showWidth = orgWidth
    	}
    	if (showWidth > orgHeight) {
    		showWidth = orgHeight
    	}
    	var imgName = url.split(imgTypes)[0]+"_"+Math.floor(showWidth)+"_"+Math.floor(showWidth);
...
//压缩图片  并且做变形处理
function getImgByThisSize(url,showWidth,showHieght,isMineBg){
	if(url.toString().indexOf("http") == 0){
	    ...
	    return getImgSizeBySize(url,showWidth,showHieght,isMineBg);
	}
}

function getImgByThisSize_chat(url,showWidth,showHieght){
	if(url.indexOf("http") == 0){
		if(url.indexOf("/guoguo/") != (-1)){
	        var s1=url.split("/guoguo/");
	    }else if(url.indexOf("/storage/") != (-1)){
...
function changeImgBySize(url,showWidth){
    ...
    if(showWidth){
    	...
    	var thisWidth = (orgWidth/2)>101?(orgWidth/2):101;
    	var imgName = url.split(imgTypes)[0]+"_"+thisWidth+"_0";
    	url = imgName+imgTypes;
    	return url;
    }
...
//截取照片
function getImgSizeBySize(url,showWidth,showHieght,isMineBg){
    ...
    if(thumSize.h > h){
        ...
        //margin-left: 17px;margin-right: 17px
       	if (over == 0) {
        	var obj = {url:url,style:"margin-left:"+over+"; margin-right:"+over+";height:"+showHieght+"px;",height:showHieght};
        }else{
        	var obj = {url:url,style:"margin-left:"+over+"px"+"; margin-right:"+over+"px;height:"+showHieght+"px;",height:showHieght};
        }
        if (isMineBg) {
        	over = over-2;
        	var obj = {style:"background-image:url("+url+");background-position:"+over+"px 0;background-size:"+(w+4)+"px "+showHieght+"px;"};
        }
        ...
        if (over == 0) {
        	var obj = {url:url,style:"margin-top:"+over+";margin-bottom:"+over+";width:"+showWidth+"px;",width:showWidth};
        }else{
        	var obj = {url:url,style:"margin-top:"+over+"px"+";margin-bottom:"+over+"px;width:"+showWidth+"px;",width:showWidth};
        }
        if (isMineBg) {
        	var obj = {style:"background-image:url("+url+");background-position:-2px "+over+"px;background-size:"+(showWidth+4)+"px "+h+"px;"};
        }
		...
		Multiplechoice.choice({
			...
			if(systemType == "ios"){
                for(var i = 0;i<ret.allimgjs.length;i++){
                    var obj={path:ret.allimgjs[i],thumbPath:ret.allimgjs[i]};
                    realResult.push(obj);
                }
            }else {
...
function openNewOfIndex(name,url,pageParam){
	api.openWin({
	    ...
	    bounces : false,
	    url: url,
	    bgColor : (getUserInfo()&&getUserInfo().item == 2)? "#232226":"#fff",
	    pageParam: pageParam,
	    animation:{
...
function bindPush(){
    ...
    if(api.systemType == "ios"){
        ...
        ajpush.bindAliasAndTags({alias: userId}, function (ret) {
            // console.log(JSON.stringify(JSON.stringify(ret) + "#@!!@#@!#!#!@#!#!@#!@#!@#!@#!@#!@#!@#!@#"))
            if (ret.statusCode == 0) {
