...
var formatTimeToNum = function(time){
...
var formatTimeToMouth = function(time){
...
function matchPassword(self){
	self.value = self.value.replace(/[\u4e00-\u9fa5 ]+/,'')
}
...
function getTimeByCtime(time){
	var thisDate = new Date();
	var year_now = thisDate.getFullYear();
	var month_now = thisDate.getMonth();
	var day_now = thisDate.getDate();
	var hours_now = thisDate.getHours();
	var minutes_now = thisDate.getMinutes();
	var seconds_now = thisDate.getSeconds();
	
	time = new Date(time)
	var year_time = time.getFullYear();
	var month_time = time.getMonth();
	var day_time = time.getDate();
	var hours_time = time.getHours();
	var minutes_time = time.getMinutes();
	var seconds_time = time.getSeconds();
	
	
	if (year_now > year_time) {
		thisTime = year_now - year_time+"年前";
	}else if (month_now > month_time) {
		thisTime = month_now - month_time + "个月前";
	}else if (day_now > day_time) {
		thisTime = day_now - day_time;
		if (thisTime < 2) {
			thisTime = "昨天";
		}else{
			thisTime = thisTime+ "天前";
		}
	}else if (hours_now > hours_time) {
		thisTime = hours_now - hours_time + "小时前";
	}else if (minutes_now > minutes_time) {
		thisTime = minutes_now - minutes_time + "min前";
	}else if (seconds_now > seconds_time) {
		thisTime = seconds_now - seconds_time + "秒前";
	}
	return thisTime;
}
...
function getHtml(row){
    ...
    if(row.type == 0){
        ...
        html += '<b >累积储值</b>达到<font color="red">' + row.totalCoin + "</font>问币";
    }else if(row.type == 2){
        if(row.questionType == 0){
            html += "等级达到<font color='red'>" + row.level + "</font>及评分维持在<font color='red'>" + row.score + "</font>分";
        }else{
            html += '<b >解答</b>附近的问题超过<font color="red">' + row.number + "</font>条";
        }
    }else if(row.type == 3){
        if(row.publishNumber != 0){
            html += "发问(已解答)总数量达到<font color='red'>" + row.publishNumber + "</font>条";
        }else if(row.attendNumber != 0){
            html += "参与(已解答)总数量达到<font color='red'>" + row.attendNumber + "</font>条";
        }else if(row.number != 0){
            html += "回答(已解答)总数量达到<font color='red'>" + row.number + "</font>条";
        }else if(row.reward != 0){
            html += "对话(已解答)内单次数打赏达到<font color='red'>" + row.reward + "</font>问币或以上";
        }else if(row.coin != 0){
            html += "发问(已解答)单次数酬金达到<font color='red'>" + row.coin + "</font>问币或以上";
        }else if(row.hasBuy == 0){
            html += "在未储值条件下、钱包累积达到<font color='red'>" + row.totalCoin + "</font>问币或以上";

        }
    }else if(row.type == 4 || row.type == 5 || row.type == 6 || row.type == 7 || row.type == 8 ){
        if(row.id == 19){
            html += "解锁吐槽之霸、整蛊专家、瞎扯大王";
        }else if(row.id == 57){
            html += "解锁科举状元、生活博士、技术宅、知识之泉";
        }else if(row.id == 56){
            html += "在[知识]类" + '<b >解答</b>超过<font color="red">' + row.number + '</font>' + "条";
        }else if(row.attendNumber != 0){
            html += "在[" + row.postTypeName + "]细分内" + '<b >参与</b>超过<font color="red">' + row.attendNumber + '</font>' + "条";
        }else if(row.number != 0){
            html += "在[" + row.postTypeName + "]细分内" + '<b >解答</b>超过<font color="red">' + row.number + '</font>' + "条";
        }
    }else if(row.type == 9){
        if(row.sex == 2 && row.number != 0){
            html += "<b >解答</b>女用户问题达到<font color='red'>" + row.number + '</font>' + "条";
        }else if(row.attendNumber != 0){
            html += "在[" + row.postTypeName + "]细分内" + '<b >参与</b>超过<font color="red">' + row.attendNumber + '</font>' + "条";
        }else if(row.number != 0){
            html += "在[" + row.postTypeName + "]细分内" + '<b >解答</b>超过<font color="red">' + row.number + '</font>' + "条";
        ...
        if(row.attendNumber != 0){
            html += "在[" + row.postTypeName + "]细分内" + '<b >参与</b>超过<font color="red">' + row.attendNumber + '</font>' + "条";
        }else if(row.number != 0){
            html += "在[" + row.postTypeName + "]细分内" + '<b >解答</b>超过<font color="red">' + row.number + '</font>' + "条";
        }else if(row.sex == 2 && row.attendNumber != 0 && row.name == "金刚芭比"){
            html += "在男士[" + row.postTypeName + "]细分内" + '<b >解答</b>超过<font color="red">' + row.attendNumber + '</font>' + "条";
        }
    }
    return html;
}