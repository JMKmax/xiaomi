...
(function($, document) {
	...
	var PopPicker = $.PopPicker = $.Class.extend({
		...
		init: function(options) {
			...
			if(self.options.noLinkage){
				self._createPicker_noLinkage();
			}else {
				self._createPicker();
			}
		...
		// 不联动
		_createPicker_noLinkage: function() {
			var self = this;
			var layer = self.options.layer || 1;
			var width = (100 / layer) + '%';
			self.pickers = [];
		...
		//不联动 填充数据
		setData_noLinkage: function(data) {
			var self = this;
			data = data || [];
			self.pickers.forEach(function (item,index) {
				item.setItems(data[index]);
			})
		},
		//填充数据
		...
		getSelectedItems: function() {
			var self = this;
			var items = [];
			for (var picker of self.pickers) {
